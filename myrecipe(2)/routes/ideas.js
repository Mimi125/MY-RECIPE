...
var multer = require('multer');
  ...
  const upload = multer({
    storage: multer.diskStorage({
      destination: function (req, file, cb) { // 파일을 받아서 multer를 이용해서 views/uploads/경로에 파일을 저장한다.
        cb(null, 'views/uploads/');
      },
      filename: function (req, file, cb) {
        cb(null, file.originalname); // 암호화 된 파일 이름을 본래 이름으로 바꿔서 저장한다.
      }
    }),
  });
  ...
  // multer의 upload를 이용해서, 이미지 필드들을 저장한다.
